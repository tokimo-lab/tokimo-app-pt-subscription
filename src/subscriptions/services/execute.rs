...
pub async fn execute_subscription(state: &Arc<AppState>, sub_id: &str) -> bool {
    ...
    sub_handler::append_log(&storage, sub_id, &run_id, "start", &format!("开始执行: {}", sub.title), None).await;
    ...
    sub_handler::append_log(&storage, sub_id, &run_id, "searching", &format!("关键词: {}", keywords.join(", ")), None).await;
    ...
    let filter_categories: Vec<String> = sub
        .category
        .as_ref()
        .map(|c| vec![c.clone()])
        .unwrap_or_default();
    ...
    for keyword in &keywords {
        ...
        for summary in &resp.site_summaries {
            sub_handler::append_log(
                &storage, sub_id, &run_id, "searching",
        ...
        for r in resp.results {
            if !all_results.iter().any(|(x, _)| {
                x.id == r.result.id && x.title == r.result.title
            }) {
    ...
    sub_handler::append_log(
        &storage, sub_id, &run_id, "searching",
        &format!("搜索到 {} 个种子", all_results.len()), None,
    ).await;
    ...
    let ranked = rank_torrents(&all_results.iter().map(|(r, _)| r.clone()).collect::<Vec<_>>(), &prefs, sub.max_downloads_per_run as usize);

    sub_handler::append_log(
        &storage, sub_id, &run_id, "filtering",
        &format!("过滤后 {} 个候选", ranked.len()), None,
    ).await;
        ...
        match DownloadClientRepo::get_by_id(&state.db, &cid_str).await {
            ...
            Ok(None) => {
                ...
                match DownloadClientRepo::list(&state.db).await {
                    ...
                    Err(e) => {
                        sub_handler::append_log(&storage, sub_id, &run_id, "error", &format!("获取下载器失败: {e}"), None).await;
                        return false;
                    }
                }
            }
            Err(e) => {
                sub_handler::append_log(&storage, sub_id, &run_id, "error", &format!("查询下载器失败: {e}"), None).await;
                return false;
            }
        }
    } else {
    ...
    let use_torrent_file = download_url.contains("/api/torrent/genDlToken")
        || download_url.contains("/api/");
    ...
    let options = if use_torrent_file {
        ...
        let gen_token_url = format!("{}/api/torrent/genDlToken", sites.iter()
        ...
        let req = no_redirect_client.post(&gen_token_url)
                                    ...
                                    Ok(torrent_resp) => {
                                        if torrent_resp.status().is_success() {
                                            match torrent_resp.bytes().await {
                                                ...
                                                Err(e) => {
                                                    sub_handler::append_log(&storage, sub_id, &run_id, "error", &format!("读取种子文件失败: {e}"), None).await;
                                                    return false;
                                                }
                                            }
                                        } else {
                                            sub_handler::append_log(&storage, sub_id, &run_id, "error", &format!("下载种子文件失败: HTTP {}", torrent_resp.status()), None).await;
                                            return false;
                                        }
                                    }
                                    Err(e) => {
                                        sub_handler::append_log(&storage, sub_id, &run_id, "error", &format!("下载种子文件请求失败: {e}"), None).await;
                                        return false;
                                    }
                                }
                            } else {
                                sub_handler::append_log(&storage, sub_id, &run_id, "error", "API 返回的下载链接为空", Some(serde_json::json!({ "response": json }))).await;
                                return false;
                            }
                        }
                        Err(e) => {
                            sub_handler::append_log(&storage, sub_id, &run_id, "error", &format!("解析 API 响应失败: {e}"), None).await;
                            return false;
                        }
                    }
                } else {
                    sub_handler::append_log(&storage, sub_id, &run_id, "error", &format!("获取下载令牌失败: HTTP {}", resp.status()), None).await;
                    return false;
                }
            }
            Err(e) => {
                sub_handler::append_log(&storage, sub_id, &run_id, "error", &format!("获取下载令牌请求失败: {e}"), None).await;
                return false;
            }
        }
    } else {
    ...
    match DownloadClientService::add_torrent(&state.db, &client_id, options, &state.http_client).await {
        Ok(()) => {
            ...
            if let Ok(Some(model)) = subscriptions::Entity::find_by_id(uid)
                .one(&state.db)
                .await
            {
