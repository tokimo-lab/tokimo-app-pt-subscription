...
const AVAILABLE_SITES: &[SiteInfo] = &[
    ...
    SiteInfo {
        id: "m-team",
        name: "馒头",
        domain: "https://api.m-team.cc/",
        allow_auth_type: &["api_key"],
        adult_only: false,
    },
    SiteInfo {
        id: "mikanani",
...
pub async fn all_status(State(state): State<Arc<AppState>>, _auth: AuthUser) -> Response {
    match get_all_site_statuses(&state).await {
        Ok(statuses) => ok(statuses).into_response(),
        Err(e) => e.into_response(),
    }
...
pub async fn get_site_status(State(state): State<Arc<AppState>>, _auth: AuthUser, Path(id): Path<String>) -> Response {
