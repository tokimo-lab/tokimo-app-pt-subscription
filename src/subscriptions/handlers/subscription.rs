...
struct SubscriptionDebugInfo {
    subscription: crate::subscriptions::repos::subscription_repo::SubscriptionDto,
    total_runs: usize,
...
fn subscription_log_key(subscription_id: &str) -> String {
    format!("logs/subscription/{subscription_id}.log")
}

fn format_log_line(timestamp: &str, phase: &str, message: &str) -> String {
    let phase_upper = phase.to_uppercase();
    let color = match phase_upper.as_str() {
        "START" => "\x1b[36m",       // cyan
        "SEARCHING" => "\x1b[33m",   // yellow
        "FILTERING" => "\x1b[35m",   // magenta
        "MATCHING" => "\x1b[32m",    // green
        "DOWNLOADING" => "\x1b[34m", // blue
        "COMPLETED" => "\x1b[1;32m", // bold green
        "ERROR" => "\x1b[1;31m",     // bold red
        _ => "",
    };
    let reset = "\x1b[0m";
    let dim = "\x1b[2m";
    format!("{dim}[{timestamp}]{reset} {color}[{phase_upper}]{reset} {message}\n")
}
...
pub async fn append_log(
    ...
    let line = format_log_line(&timestamp, phase, message);
    ...
    match SubscriptionRepo::create(&state.db, body, &auth.user_id).await {
        Ok(sub) => {
            ...
            tokio::spawn(async move {
                crate::subscriptions::services::execute::execute_subscription(&state_clone, &sub_id).await;
...
pub async fn update(
    ...
    match SubscriptionRepo::get_raw(&state.db, &id).await {
        Ok(Some(raw)) => {
            ...
            // Prevent status changes when subscription is in terminal state
            if raw.status == "pushed" && body.status.is_some() && body.status.as_deref() != Some("pushed") {
                return AppError::BadRequest("订阅已推送下载，无法修改状态".into()).into_response();
            }
        }
...
pub async fn execute(State(state): State<Arc<AppState>>, auth: AuthUser, Path(id): Path<String>) -> Response {
    match SubscriptionRepo::get_raw(&state.db, &id).await {
        Ok(Some(raw)) => {
            ...
            // Prevent execution when subscription is in terminal state
            if raw.status == "pushed" {
                return AppError::BadRequest("订阅已推送下载，无法再次执行".into()).into_response();
            }
        }
        Ok(None) => return AppError::NotFound("订阅不存在".into()).into_response(),
        Err(e) => return e.into_response(),
...
pub async fn get_raw_logs(State(state): State<Arc<AppState>>, auth: AuthUser, Path(id): Path<String>) -> Response {
    ...
    let key = subscription_log_key(&id);
    let content = state.storage().download(&key).await.unwrap_or_default();
